...
Usage (standalone):
    from openclip_core.subtitle_burner import SubtitleBurner
...
from openclip_core.config import (
    API_KEY_ENV_VARS,
    SUBTITLE_TRANSLATION_LAUNCH_STAGGER_SECONDS,
    SUBTITLE_TRANSLATION_MAX_WORKERS,
)
from openclip_core.font_utils import build_missing_font_message, find_best_font
    ...
    def __init__(
        ...
        if enable_llm:
            ...
            if provider == "openrouter":
                from openclip_core.llm.openrouter_api_client import OpenRouterAPIClient
                self.client = OpenRouterAPIClient(api_key=api_key, base_url=base_url)
            elif provider == "glm":
                from openclip_core.llm.glm_api_client import GLMAPIClient
                self.client = GLMAPIClient(api_key=api_key, base_url=base_url)
            elif provider == "minimax":
                from openclip_core.llm.minimax_api_client import MiniMaxAPIClient
                self.client = MiniMaxAPIClient(api_key=api_key, base_url=base_url)
            elif provider == "custom_openai":
                from openclip_core.llm.custom_openai_api_client import CustomOpenAIAPIClient
                self.client = CustomOpenAIAPIClient(api_key=api_key, base_url=base_url)
            else:
                from openclip_core.llm.qwen_api_client import QwenAPIClient
